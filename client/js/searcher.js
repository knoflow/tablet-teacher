Searcher = Class.extend({
	init: function(useSocket) {
		if(useSocket) this.socket = io.connect("http://localhost:5000/search");
	},
	...
	performAjaxSearch: function(searchType, keywords) {
		var _this = this;
		$.get('http://localhost:5000/search', {type: searchType, keywords: keywords}, function(searchResultsJson) {
			_this.updatePage(searchType, searchResultsJson);	
		});
	},
